...
//Reading a block of data from stdin
void readStdinBlockData(unsigned int num_samples, unsigned int block_id, std::vector<float> &block_data)
{
	std::vector<char> raw_data(num_samples);
	std::cerr << "Reading " << num_samples << " samples from stdin for block " << block_id << std::endl;
	std::cin.read(reinterpret_cast<char*>(&raw_data[0]), num_samples*sizeof(char));
	//std::cout << "Hello" << std::endl;
	for(int k = 0; k<(int) num_samples;k++)
	{
		//std::cerr << "Sample " << k << " is " << (int)raw_data[k] << std::endl;
