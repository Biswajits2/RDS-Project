#Sai Satwika Kurra and Khawja Labib
#Febuary 08, 2024
#Lab 3
...
# use fmDemodArctan and fmPlotPSD and my_demodArctan
from fmSupportLib import fmDemodArctan, fmPlotPSD, my_demodArctan
# for take-home add your functions

#Low pass function from past labs (same as fmMonoBasic)
def impulseLPF(Fc, Fs, Ntaps):
    
    norm_cutoff = Fc / (Fs / 2) #cutoff frequency
    h = np.zeros(Ntaps)
    ...
    for i in range (Ntaps): 
        if i==((Ntaps-1)/2):
            ...
            h[i] = norm_cutoff*np.sin(np.pi*norm_cutoff*(i-(Ntaps-1)/2))/(np.pi*norm_cutoff*(i-(Ntaps-1)/2))
           
        h[i] *= (np.sin(i * np.pi/Ntaps))**2
        
    return h

#Convolution function from Lab 2
def convolution(a,b, state): 
	x = np.zeros(len(b))
...
if __name__ == "__main__":
	...
	rf_coeff = impulseLPF(rf_Fc, rf_Fs, rf_taps)
	...
	if il_vs_th == 0:
		...
		#audio_coeff = np.array([])
		audio_coeff = impulseLPF(audio_Fc, 240e3, audio_taps)		
	...
	buffer_i = 0
	buffer_q = 0
	initial_state = {'buffer_i': 0, 'buffer_q': 0}
	mono_state = np.zeros(audio_taps - 1)
	...
	while (block_count+1)*block_size < len(iq_data):
		...
		if il_vs_th == 0:
			...
			fm_demod, buffer_i, buffer_q = my_demodArctan(i_ds, q_ds, buffer_i, buffer_q)
		...
		# 	# same principle as for i_filt or q_filt (but different arguments)
		 	audio_filt,state_mono = signal.lfilter(audio_coeff, 1.0, fm_demod,zi=mono_state)
		else:
		# 	# to be updated by you for the takehome exercise
		# 	# with your own code for BLOCK convolution
		 	audio_filt, state_mono = convolution(audio_coeff, fm_demod, mono_state)
